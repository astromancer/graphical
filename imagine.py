...
class ImageDisplay(object):
    ...
    # TODO: Sliders as shaded horizontal area (Rectangle?)

    # TODO: Subclass AstroImageDisplay
    # TODO: Option for orientation markers - NE arrows

    # FIXME: Dragging too slow!!
    # FIXME: sliders disappear behind histogram (set zorder / redraw slide marers last)
    # FIXME: BINARY DATA
    # FIXME: MASKED DATA
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def __init__(self, data, *args, **kwargs):
        ...
        self.use_blit = kwargs.pop('use_blit', False)       # FIXME: blit broken
        ...
        kwargs = self.update_colourscale_limits(data, **kwargs)
        ...
        if autoscale_figure:
            ...
            self.figure.subplots_adjust(left=0.05, right=0.95, top=0.98, bottom=0.01)
        ...
        self.cid = ax.figure.canvas.mpl_connect('draw_event', self._on_first_draw)
        self.connect()
    ...
    def _on_draw(self, event):
        print('DRAW', vars(event))

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def guess_figsize(self, data=None):
        shape = self.data.shape if data is None else np.shape(data)
        screensize = 13, 16.5     # max scale #TODO: automated way of getting this?
        fillFactor = 0.75       #TODO max fill factor in each direction
        figsize = (shape / np.max(shape))[::-1] * fillFactor * screensize[np.argmax(shape)]
        return figsize
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def update_colourscale_limits(self, data, **kwargs):
        """
        ...
        Keywords
        --------
        colorscale       :       str - {'percentile', 'zscale'};
                                default 'percentile'
        ...
        if ('vmin' in kwargs) and ('vmax' in kwargs):
            ...
            return kwargs               # NOTE: might still need to pop cruft as below

        # clims means the same as vmin, vmax
        clims = kwargs.pop('clims', None)
        if clims is None:
            self.colourScale = cs = kwargs.pop('colorscale', 'percentile')

            if cs.startswith('p'):
                pmin = kwargs.pop('pmin', 2.25)
                pmax = kwargs.pop('pmax', 99.75)
                plims = kwargs.pop('plims', (pmin, pmax))       # TODO: dynamically update.  Display next to sliders??
                clims = np.percentile(data, plims)

            elif cs.startswith('z'):
    ...
    update_colorscale_limits = update_colourscale_limits

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def get_colourscale_limits(self, data, **kwargs):
        """Get colour scale limits for data"""
        kws = self.update_colourscale_limits(data, **kwargs)
    ...
    def createColorbar(self):
        ...
        if self.has_hist:
            # No need for the data labels on the
            from matplotlib import ticker
            self.cax.yaxis.set_major_formatter(ticker.NullFormatter())
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def createHistogram(self, ax):  # TODO: as a separate class SideBar
        """histogram data on slider axis"""
        # from matplotlib.collections import PatchCollection

        h = ax.hist(self._sanitize_data(self.data), bins=100,
                    orientation='horizontal', log=True)
        self.hvals, self.bin_edges, self.patches = h

        # TODO: use PatchCollection?????
        # FIXME: Not displaying on initial draw with use_blit = True
        if self.use_blit:
    ...
    def get_hcol(self, clims):
        ...
        if np.ma.is_masked(vm):
            colours[vm.mask, :3] = 0.25  # grey out histogram patches outside of colour range
            colours[vm.mask, -1] = 1
        return colours
    ...
    def set_clim(self, xydata):
        ...
        if self.sliders.selection is not None:  # explicit call (not generated by moving a slider with the mouse)
            clims[self.sliders.index()] = xydata[self.sliders._index]
        self.imgplt.set_clim(clims)

        if not self.has_hist:
            return self.imgplt  # TODO: COLOURBAR ticklabels??

        self.updateHistogram(clims)
        return self.imgplt, self.patches  # TODO: COLOURBAR ticklabels??
...
class CubeDisplayBase(ImageDisplay):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def __init__(self, data, coords=None, **kwargs):
        """
        ...
        """
        # setup image display
        ImageDisplay.__init__(self, data, **kwargs)
    ...
    def adjust_figure_to_data_size(self, data=None):
        figsize = self.guess_figsize(data)
        figsize *= (1,  1.25)  # make the figure slightly taller (room for slider)
        self.ax.figure.set_size_inches(figsize)
    ...
    def set_frame(self, i, draw=False):
        ...
        if self.colourScale:
            ...
            vmin, vmax = self.get_colourscale_limits(data, colorscale=self.colourScale)
...
class ImageCubeDisplay(CubeDisplayBase):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def __init__(self, data, coords=None, **kwargs):
        CubeDisplayBase.__init__(self, data[0], **kwargs)
...
class ImageCubeDisplayA(ImageCubeDisplay):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def __init__(self, data, ap_prop_dict={}, ap_updater=None, **kwargs):
        """with Apertures
        ...
        """
        CubeDisplayBase.__init__(self, data, **kwargs)
        ...
        # check updater
        if (not ap_updater is None) and (not isinstance(ap_updater, Callable)):
            raise TypeError
        # else:
...
class ImageCubeDisplayX(ImageCubeDisplay):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def __init__(self, data, coords=None, **kwargs):
        CubeDisplayBase.__init__(self, data, coords, **kwargs)
...
class FitsCubeDisplay(ImageCubeDisplayA, FitsCube):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def __init__(self, filename, ap_prop_dict={}, ap_update_dict={}, **kws):
        """ """
        # setup data access
        FitsCube.__init__(self, filename)
        extent = np.c_[(0, 0), self.ishape[::-1]].flatten() - 0.5
        ImageCubeDisplayA.__init__(self, self[0],
                                   ap_prop_dict, ap_update_dict,
                                   origin='llc', extent=extent,
...
class Compare3DImage():
    ...
    def __init__(self, *args, **kws):
        """
        args=X, Y, Z, data  or  fig, X, Y, Z, data
        """
    ...
    def setup_figure(self, fig=None):
        ...
        # TODO:  Does not display correctlt with tight layout
...
def supershow(ax, data, *args, **kwargs):
    ss = ImageDisplay(data, *args, ax=ax, **kwargs)
    ss.connect()
    return ss.imgplt
